...
def showPlot2():
    ...
    x_axis = []
    for x in room_size:
        y = runSimulation(2,1.0,x[0],x[1],.8,StandardRobot)
        y_axis.append(y)
        x_axis.append(x[0])
    pylab.plot(x_axis,y_axis,'bo')
...
def showPlot3():
    ...
    y_ax_r2 = []
    for x in range(1,11):
        y_r1 = runSimulation(x,1.0,20,20,.8,10,StandardRobot)
        y_r2 = runSimulation(x,1.0,20,20,.8,10,RandomWalkRobot)
        x_axis.append(x)
        y_ax_r1.append(y_r1)
        y_ax_r2.append(y_r2)
    pylab.plot(x_axis,y_ax_r1,'bo')
    pylab.plot(x_axis,y_ax_r2,'ro')
    pylab.title('Compares time taken to clean a room between standard robots(blue) and random robots(red)')
